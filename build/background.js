...
// Updates the available go links for redirect use

function refreshCache(next = () => {}) {
  console.log('Updating go link cache');
  chrome.storage.sync.get(['goLinks'], ({ goLinks }) => {
    ...
    redirects = goLinks
      .map(link => {
        return link.aliases.reduce(
          (acc, alias) => Object.assign(acc, { [alias]: link.link }),
          {},
        );
      })
      .reduce((acc, mappedAliases) => Object.assign(acc, mappedAliases), {});
    next();
  });
}

// Tells the script to update the cache

chrome.runtime.onMessage.addListener(message => {
  switch (message) {
    case 'update-go-link-cache':
      return refreshCache();
  }
});

// Redirect - meat and potatoes

chrome.webRequest.onBeforeRequest.addListener(
  details => {
    const pathSegments = new URL(details.url).pathname.substring(1).split(/\//);

    console.log('Attempting redirect for:', pathSegments);

    if (!redirects[pathSegments[0]]) {
      console.log(`No redirect link found: `, redirects);
      return { redirectUrl: chrome.extension.getURL('index.html') };
    }

    console.log('Redirecting to: ', redirects[pathSegments[0]]);
    return {
      redirectUrl: redirects[pathSegments[0]],
    };
  },
  { urls: ['*://go/*'] },
  ['blocking'],
...
chrome.commands.onCommand.addListener(command => {
  switch (command) {
    case 'open-go-link-tab':
      return chrome.tabs.create({ url: chrome.extension.getURL('index.html') });
  }
});

chrome.omnibox.onInputChanged.addListener((text, callback) => {
  refreshCache(() => {
    const suggestions = new Fuse(links, fuzzySearchConfig)
      .search(text)
      .flatMap(link =>
...
chrome.omnibox.onInputEntered.addListener((text, disposition) => {
  refreshCache(() => {
    console.log(text, '=>', redirects[text] || 'https://go?text=' + text);
    switch (disposition) {
      case 'currentTab':
        return chrome.tabs.update({
          url: redirects[text] || 'https://go?text=' + text,
        });
